...
export async function POST(request: Request) {
  ...
  if (action === 'message') {
    ...
    const mediaCandidate = clean(body.mediaData, 1300000)
    ...
    if (mediaCandidate) {
      ...
      if (mediaCandidate.length > 1250000) return NextResponse.json({ error: 'Attachment is too large. Please use a smaller photo or shorter voice note.' }, { status: 400 })
    }
    const result = await pool.query(
