...
export default function Page() {
  ...
  return (
    <main className="app-page public-page" data-theme={theme} data-mode={experience}>
      ...
      <header className="topbar public-topbar"><button className="brand wordmark" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}><Mail size={16} /> FOWZAN&apos;S INBOX</button><div className="topbar-controls"><button className="appearance-button" onClick={() => setAppearanceOpen(true)} aria-label={experience === "gamer" ? "Gamer mode" : experience === "professional" ? "Minimal mode" : "Choose appearance"} title={experience === "gamer" ? "Gamer mode" : experience === "professional" ? "Minimal mode" : "Choose appearance"}>{experience === "gamer" ? <Gamepad2 size={15} /> : experience === "professional" ? <PenLine size={15} /> : <Sparkles size={14} />}<span>{experience === "professional" ? "Minimal" : experience ? "Gamer" : "Appearance"}</span></button>{experience && <label className="theme-picker" title="Choose color"><Palette size={15} /><span>COLOR</span><select aria-label="Choose color theme" value={theme} onChange={(event) => handleAppearanceChange(experience, event.target.value as typeof theme)}>{(experience === "gamer" ? ["purple","red","green","rose","blue","white"] : ["rose","blue","purple","mono"]).map((accent) => <option key={accent} value={accent}>{accent.toUpperCase()}</option>)}</select></label>}<button className="private-button" onClick={() => setView('private')} aria-label="Open private inbox" title="Private inbox"><LockKeyhole size={15} /><span>private inbox</span></button></div></header>
