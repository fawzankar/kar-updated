...
export default function Page() {
          ...
          <div className="thread-list">
            {visibleThoughts.map((item) => (
              <div key={item.id} className={`thread-item-row ${selected?.id === item.id ? 'active' : ''}`}>
                ...
                <button className="thread-item-share" onClick={(event) => { event.stopPropagation(); void shareStoryImage(item) }} aria-label="Share this question as a story" title="Share this question as a story">
  ...
  return (
    <main className="app-page public-page" data-theme={theme} data-mode={experience}>
      <div className="cyber-bg" aria-hidden="true"><i /><i /><i /><div className="space-stars">{Array.from({ length: 36 }, (_, index) => <span key={index} style={{ top: `${(index * 47) % 100}%`, left: `${(index * 73 + 11) % 100}%`, animationDelay: `${-(index % 17) * 0.42}s`, animationDuration: `${4.5 + (index % 7) * 0.8}s` }} />)}</div><div className="asteroid-field">{Array.from({ length: 5 }, (_, index) => <i key={index} style={{ top: `${(index * 31 + 8) % 94}%`, left: `${(index * 61 - 12) % 112 - 4}%`, animationDelay: `${-(index % 9) * 1.15}s`, animationDuration: `${18 + (index % 6) * 2.1}s`, transform: `scale(${0.7 + (index % 5) * 0.22}) rotate(${(index * 23) % 360}deg)` }} />)}</div>{Array.from({ length: 42 }, (_, index) => matrixStreams[index % matrixStreams.length]).map((stream, index) => <b key={index} style={{ left: `${(index * 2.37 + 1.5) % 98}%`, animationDelay: `${-((index * 0.71) % 12)}s`, opacity: `${0.28 + (index % 5) * 0.055}` }}>{Array.from(stream).map((char, charIndex) => <span key={charIndex}>{char}</span>)}</b>)}</div><div className="ambient-orb orb-a" /><div className="ambient-orb orb-b" /><div className="ambient-orb orb-d" />
      <div className="particle-field" aria-hidden="true">{Array.from({ length: 52 }, (_, index) => <i key={index} style={{ top: `${(index * 37 + 7) % 96}%`, left: `${(index * 61 + 13) % 98}%`, animationDelay: `${-((index * 0.47) % 13)}s`, animationDuration: `${7 + (index % 7) * 1.1}s`, transform: `scale(${0.65 + (index % 5) * 0.22})` }} />)}</div>
