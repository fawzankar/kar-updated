...
export default function Page() {
  ...
  const [ownerMediaModes, setOwnerMediaModes] = useState<Record<number, 'image' | 'gif' | 'audio' | null>>({})
  const [ownerMediaData, setOwnerMediaData] = useState<Record<number, string>>({})
  ...
  const ownerRecordingThreadRef = useRef<number | null>(null)
  const ownerRecordingStartedAtRef = useRef(0)
  ...
  async function toggleOwnerRecording(id: number) {
    if (ownerRecordingThreadRef.current === id && ownerRecorderRef.current) { try { ownerRecorderRef.current.stop() } catch {}; return }
    if (ownerRecordingThreadRef.current !== null) { try { ownerRecorderRef.current?.stop() } catch {}; ownerStreamRef.current?.getTracks().forEach((track) => track.stop()); ownerRecordingThreadRef.current = null; ownerRecorderRef.current = null }
    if (!window.isSecureContext && window.location.hostname !== 'localhost') { setError('Voice notes require HTTPS. Open the secure Vercel URL.'); return }
    ...
    try {
      ...
      ownerStreamRef.current = stream; ownerRecorderRef.current = recorder; ownerRecordingThreadRef.current = id; ownerChunksRef.current = []; ownerRecordingStartedAtRef.current = Date.now()
      recorder.ondataavailable = (event) => { if (event.data?.size) ownerChunksRef.current.push(event.data) }
      recorder.onerror = () => { stream.getTracks().forEach((track) => track.stop()); ownerStreamRef.current = null; ownerRecorderRef.current = null; ownerRecordingThreadRef.current = null; setError('The browser could not record that voice note.') }
      recorder.onstop = async () => {
        stream.getTracks().forEach((track) => track.stop()); ownerStreamRef.current = null; ownerRecorderRef.current = null; ownerRecordingThreadRef.current = null
        const elapsed = Date.now() - ownerRecordingStartedAtRef.current
        const blob = new Blob(ownerChunksRef.current, { type: recorder.mimeType || mimeType || 'audio/webm' }); ownerChunksRef.current = []
        if (elapsed < 300 || blob.size < 200) { setError('The voice note was too short. Hold record for a moment and try again.'); return }
        if (blob.size > 900000) { setError('That voice note is too large. Please record a shorter note.'); return }
        try { const data = await readFileAsDataUrl(new File([blob], 'fowzan-voice-note', { type: blob.type || 'audio/webm' }), 900000); setOwnerMediaData((current) => ({ ...current, [id]: data })); setOwnerMediaUrls((current) => ({ ...current, [id]: '' })); setOwnerMediaModes((current) => ({ ...current, [id]: 'audio' })) }
        catch (err) { setError(err instanceof Error ? err.message : 'Could not prepare the voice note.') }
      }
      recorder.start(250)
      window.setTimeout(() => { if (ownerRecorderRef.current === recorder && recorder.state === 'recording') recorder.stop() }, 60000)
    } catch (err) { ownerStreamRef.current?.getTracks().forEach((track) => track.stop()); ownerStreamRef.current = null; ownerRecorderRef.current = null; ownerRecordingThreadRef.current = null; setError(err instanceof DOMException && err.name === 'NotAllowedError' ? 'Microphone access was blocked. Allow the microphone and try again.' : 'Microphone access was not granted.') }
  }

  async function submitOwnerReply(id: number) {
    const text = ownerReplies[id]?.trim() ?? ''
    const selectedMode = ownerMediaModes[id] ?? null
    const mediaUrl = selectedMode === 'audio' ? (ownerMediaData[id] ?? '') : (ownerMediaUrls[id]?.trim() ?? '')
    const mediaType = selectedMode ?? (mediaUrl ? 'image' : null)
    ...
    try {
      ...
      localStorage.removeItem(`fowzan-draft-${id}`); setOwnerReplies((current) => ({ ...current, [id]: '' })); await loadOwner(true)
      setOwnerMediaUrls((current) => ({ ...current, [id]: '' })); setOwnerMediaData((current) => { const next = { ...current }; delete next[id]; return next }); setOwnerMediaModes((current) => ({ ...current, [id]: null }))
  ...
  async function shareThreadLink(item: Thought | PublicResponse) {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: "Fowzan's Inbox · Conversation", url })
      } else {
