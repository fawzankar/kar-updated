...
export async function POST(request: Request) {
  ...
  const result = await pool.query(`SELECT id, username, display_name AS "displayName", role, password_hash, must_change_password AS "mustChangePassword" FROM users WHERE username = $1 AND active = TRUE LIMIT 1`, [username])
...
export async function GET() {
  ...
  return NextResponse.json({ authenticated: Boolean(user), user: user ? { username: user.username, displayName: user.displayName, role: user.role, mustChangePassword: Boolean(user.mustChangePassword) } : null }, { headers: { 'Cache-Control': 'no-store' } })
}
